            ...
            >
              <div>
                ...
                <div className="relative aspect-4/3 w-full overflow-hidden rounded-2xl bg-stone-200 dark:bg-[#2a211b]">
                  ...
                  {recipe.cookTime && (
                    <span className="absolute bottom-2.5 left-2.5 rounded-lg bg-black/75 px-2 py-0.5 text-[10px] font-bold text-amber-300 backdrop-blur-sm">
                      ⏱ {recipe.cookTime}m
                    </span>
                  )}
                </div>
              ...
              {/* ACTION BUTTON WITH CRISP HOVER */}
              <div className="mt-4 pt-2.5 border-t border-stone-200/80 dark:border-white/5 flex items-center justify-between">
                <Link
                  href={`/recipes/${recipe.id}`}
                  className="w-full inline-flex items-center justify-center gap-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 py-2 px-3 text-xs font-bold text-stone-950 shadow-2xs transition active:scale-95 cursor-pointer"
                >
