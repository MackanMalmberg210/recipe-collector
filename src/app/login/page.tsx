...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Direct same-origin API request (works 100% on mobile Wi-Fi, PWA, and IP addresses)
      ...
      // Sync with Supabase client if session was returned
      if (data.session) {
        try {
    ...
    <main className="relative flex min-h-[calc(100vh-73px)] w-full items-center justify-center overflow-hidden bg-[#0a0908] px-4 py-8 lg:px-8">
      ...
      <div className="relative mx-auto grid w-full max-w-6xl items-center gap-12 lg:grid-cols-12">
        ...
        <div className="hidden lg:col-span-6 lg:flex lg:flex-col lg:justify-center">
          <div className="inline-flex items-center gap-2.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-emerald-300 backdrop-blur-md">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              >
                {loading ? (
                  ...
                  <span>{mode === "login" ? "Sign In to Kitchen" : "Create Free Account ✨"}</span>
                )}
              </button>
            </form>
